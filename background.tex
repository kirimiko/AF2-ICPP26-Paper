...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.48\textwidth]{figures/opa_new.pdf}
    \caption{4-way Outer-Product Accumulator (OPA) Instruction on 72F CPU}
    \label{fig:opa}
\end{figure}
...
To update these tiles, the unit executes a generalized \textit{$K$-way outer product}. Instead of performing a rank-1 update ($u \otimes v$), the hardware aggregates inputs to perform a rank-$K$ update in a single step. For an input vector pair, the accelerator computes the outer product of length-$K$ sub-vectors and accumulates the result into the 32-bit destination tile.
