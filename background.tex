...
The 72F processor is an ARM CPU equipped with the SME. SME extends the AArch64 architecture with a two-dimensional matrix register file (ZA registers) and dedicated outer-product execution units, enabling high-throughput outer-product accumulation (OPA) operations with hardware-managed accumulation.
...
Computation in SME is driven by a hardware outer-product unit. Given a horizontal source vector $u$ and a vertical source vector $v$, the hardware computes their outer product and accumulates the result into the destination tile:
$$
