...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.3\linewidth]{figures/opa_new.pdf}
...
The 72F processor is an ARM CPU equipped with the **Scalable Matrix Extension (SME)**. SME extends the AArch64 architecture with a two-dimensional matrix register file and dedicated outer-product execution units, enabling high-throughput matrix operations with hardware-managed accumulation.

At the core of SME is the **ZA matrix register**, a reconfigurable two-dimensional storage structure. Unlike traditional fixed-length vector registers, ZA is logically partitioned into independent accumulator tiles. The number of logical tiles depends on the element precision: narrower data types provide more tiles, while wider types reduce the number of tiles due to fixed physical storage capacity. In this work, we focus on FP32 and FP16 computation modes.

SME supports orthogonal access to the matrix register. Accumulator tiles can be accessed either by rows or by columns without explicit data rearrangement in software. This orthogonal addressing eliminates the need for software-level matrix transposition and enables efficient data reuse across different tensor dimensions, which is particularly beneficial for GEMM-like kernels.

Computation in SME is driven by a hardware outer-product unit. Rather than performing scalar fused multiply-add operations element by element, SME updates the accumulator tile using vector outer products. Given a horizontal source vector $u$ and a vertical source vector $v$, the hardware computes their outer product and accumulates the result into the destination tile:
$$
    D \leftarrow D + (u \otimes v)
$$
where $D$ denotes the FP32 accumulator tile.

For **FP32 workloads**, both input vectors and the accumulator tile use single precision. Each outer-product instruction multiplies elements from the horizontal and vertical FP32 vectors and accumulates the products directly into the FP32 tile.

For **FP16 workloads**, SME employs a widening outer-product mechanism. The input vectors are stored in FP16 format, while accumulation is performed in FP32 precision to improve numerical robustness. In this mode, the hardware performs a two-lane outer-product aggregation per instruction: pairs of adjacent FP16 elements are multiplied and accumulated into FP32 accumulators. This design increases floating-point throughput compared to pure FP32 operation while preserving numerical stability through precision widening.

By combining a two-dimensional accumulator register file, orthogonal row/column access, and precision-widening outer-product units, the SME-enabled 72F CPU efficiently supports both FP32 and FP16 matrix-intensive workloads.
